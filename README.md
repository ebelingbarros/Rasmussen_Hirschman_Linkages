...
- The chief objective of this project was to train and test my abilities using Python. I decided to pursue it around two weeks after the Data Analytics Bootcamp had started, and used time in between classes and labs to code. 
- In my Doctoral Thesis, I deal with the notion of Backward and Forward Linkages in a conceptual manner. I thus felt that it could be advantageous to also examine them empirically for follow up scientific articles.
- Calculating Rasmussen-Hirschman Backward and Forward Linkages using Excel can be both hugely challenging and frustrating. There are 42 countries in the WIOD'S (World Input Output Database) table, each of which has 56 economic sectors. Data is available for 15 years. This makes a huge volume of statistical information. Although still possible, calcualting the indexes for this amount of data with Excel would have cost a lot of time without being able to use code. Perhaps due to this reason most of the scientific papers I'm aware of calculate the index for a handful of countries. 
- My objetive was to calculate the indexes for all the possible data points to be able to create meaningful visualizations (for example with Seaborn's KDE plot) to corroborate or reject my working hypothesis that the manufacturing sector matters for economic development, and that countries with higher GDP per capita tend to have larger backward and forward linkages in those sectors than less developed countries. 
- Although the working hypothesis could not be fully corroborated, this was an invaluable exercise in my quest to become a professional Data Analyst/Data Scientist. Not only have i learned to code - using, for example, relatively complex matricial operations - but also the project has aided my objective of transitiong from a heavy Excel user to a coder. **Coding makes things so much easier!** In addition, coding **unlocks so much intellectual potential**. Once you learn to code, **you perceive that there are so many complex things that can be easily done, things that seemed impossible before**. 
...
<p align="center"><b> Backward linkages vs time </b></p> 
<p align="center">
...
<p align="center"><b> Forward linkages vs time </b></p>
<p align="center">
